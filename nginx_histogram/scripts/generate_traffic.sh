...
if [ "$MIN_SIZE" -ge "$MAX_SIZE" ]; then
  echo >&2 "Error: min_size must be less than max_size"
  exit 1
fi

echo >&2 "Starting traffic generation..."
echo >&2 "Size range: ${MIN_SIZE}-${MAX_SIZE} bytes"
...
while true; do
  ...
  echo >&2 "Sent ${SIZE} bytes to endpoint${ENDPOINT}"
  sleep 0.1
done
